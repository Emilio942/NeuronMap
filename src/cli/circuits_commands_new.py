...
logger = logging.getLogger("neuronmap.cli.circuits_new")
...
def setup_logging(verbose: bool):
    ...
    base_logger.log_system_event(
        event_type="circuits_cli_new_logging_initialized",
        message="Circuits CLI (new) logging configured",
...
@click.option('--threshold', '-t', default=0.1, help='Minimum induction score threshold')
...
@click.option('--concentration-threshold', default=2.0, help='Minimum concentration threshold')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--format', 'output_format', 
              type=click.Choice(['json', 'graphml', 'both']), 
              default='json', help='Output format')
...
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
@click.pass_context
def find_copying_heads(ctx, model: str, threshold: float, concentration_threshold: float,
                      output: Optional[str], output_format: str, device: str, verbose: bool):
    """
    ...
    try:
        ...
        click.echo(f"📊 Using thresholds: copying={threshold}, concentration={concentration_threshold}")
        ...
        test_prompt = tokenizer(test_text, return_tensors="pt")["input_ids"].to(model_obj.device)
        ...
        for i, (layer, head, scores) in enumerate(candidates[:5]):  # Show top 5
            ...
            click.echo(f"  {i+1}. Layer {layer}, Head {head} (main score: {main_score:.3f})")
...
@click.option('--threshold', '-t', default=0.3, help='Minimum composition score threshold')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--format', 'output_format', 
              type=click.Choice(['json', 'graphml', 'both']), 
              default='json', help='Output format')
...
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
@click.pass_context
def analyze_composition(ctx, model: str, layer1: int, layer2: int, threshold: float,
                       output: Optional[str], output_format: str, device: str, verbose: bool):
    """
    ...
    try:
        ...
        if output:
            ...
            output_path = Path(f"composition_{model.replace('/', '_')}_L{layer1}_L{layer2}")
...
@click.option('--threshold', '-t', default=0.1, help='Minimum influence score threshold')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--format', 'output_format', 
              type=click.Choice(['json', 'graphml', 'both']), 
              default='json', help='Output format')
...
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
@click.pass_context
def analyze_neuron_head(ctx, model: str, max_layers: int, threshold: float,
                       output: Optional[str], output_format: str, device: str, verbose: bool):
    """
...
@click.option('--target-position', default=-1, help='Target token position (-1 for last)')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--device', default='auto', help='Device to use (auto, cpu, cuda)')
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
@click.pass_context
def verify_circuit(ctx, model: str, circuit_file: str, prompt: str, target_position: int,
                  output: Optional[str], device: str, verbose: bool):
    """
    ...
    try:
        ...
        for i, (comp_id, effect_data) in enumerate(sorted_effects[:5]):
            ...
            click.echo(f"      Layer {effect_data['layer']}, Position {effect_data['position']}")
